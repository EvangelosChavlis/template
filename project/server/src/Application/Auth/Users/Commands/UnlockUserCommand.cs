...
public record UnlockUserCommand(Guid Id, Guid Version) : IRequest<Response<string>>;
...
public class UnlockUserHandler : IRequestHandler<UnlockUserCommand, Response<string>>
{
    ...
    public UnlockUserHandler(ICommonRepository commonRepository, IUnitOfWork unitOfWork)
    {
    ...
    public async Task<Response<string>> Handle(UnlockUserCommand command, CancellationToken token = default)
    {
        ...
        if (!user.IsActive)
        {
            await _unitOfWork.RollbackTransactionAsync(token);
            return new Response<string>()
                .WithMessage("Error unlocking user.")
                .WithStatusCode((int)HttpStatusCode.BadRequest)
                .WithSuccess(false)
