...
public record UpdateContinentCommand(Guid Id, UpdateContinentDto Dto) : IRequest<Response<string>>;

public class UpdateContinentHandler : IRequestHandler<UpdateContinentCommand, Response<string>>
{
    private readonly ICommonRepository _commonRepository;
    private readonly ICommonQueries _commonQueries;
    private readonly IUnitOfWork _unitOfWork;
    
    public UpdateContinentHandler(ICommonRepository commonRepository, 
    ...
    public async Task<Response<string>> Handle(UpdateContinentCommand command, CancellationToken token = default)
    {
        // Check current user
        var currentUser = await _commonQueries.GetCurrentUser(token);
        if(!currentUser.UserFound)
            return new Response<string>()
                .WithMessage("Error updating continent.")
        ...
        // Check for concurrency issues
        if (continent.Version != command.Dto.Version)
        {
            await _unitOfWork.RollbackTransactionAsync(token);
            return new Response<string>()
        ...
        command.Dto.UpdateContinentMapping(continent);
        ...
        if(!result)
        {
            ...
            return new Response<string>()
                ...
                .WithSuccess(false)
                .WithData("Failed to update continent.");
        }
        ...
        // Initializing object
        return new Response<string>()
            .WithMessage("Success updating continent.")
            .WithStatusCode((int)HttpStatusCode.Created)
            .WithSuccess(result)
            .WithData($"Continent {continent.Name} updated successfully!");
    }
}