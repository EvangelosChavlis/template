...
public record DeactivateContinentCommand(Guid Id, Guid Version) : IRequest<Response<string>>;

public class DeactivateContinentHandler : IRequestHandler<DeactivateContinentCommand, Response<string>>
{
    private readonly ICommonRepository _commonRepository;
    private readonly ICommonQueries _commonQueries;
    private readonly IUnitOfWork _unitOfWork;
    
    public DeactivateContinentHandler(ICommonRepository commonRepository, 
    ...
    public async Task<Response<string>> Handle(DeactivateContinentCommand command, CancellationToken token = default)
    {
        // Check current user
        ...
        // Check if the continent is not active
        if (!continent.IsActive)
        {
            ...
            return new Response<string>()
                ...
                .WithStatusCode((int)HttpStatusCode.BadRequest)
                .WithSuccess(false)
                .WithData("Continent is deactivated.");
        }
            
        var countriesFilters = new Expression<Func<Country, bool>>[] { c => c.ContinentId == continent.Id };
        var countriesWithContinent = await _commonRepository.AnyExistsAsync(countriesFilters, token);
        
        if (countriesWithContinent)
        {
            var countriesCounter = await _commonRepository.GetCountAsync(countriesFilters, token);
            ...
            return new Response<string>()
                ...
                .WithSuccess(false)
                .WithData($"This continent is used from {countriesCounter} and cannot be deactivated.");
        }
        ...
        return new Response<string>()
            ...
            .WithSuccess(result)
            .WithData($"Continent {continent.Name} deactivated successfully.");
    }
}