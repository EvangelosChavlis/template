...
public static class DistrictModelValidators
{
    public static ValidationResult Validate(this District model)
    {
        ...
        else if (model.Name.Length > DistrictLength.NameLength)
            errors.Add($"Name {model.Name} must not exceed {DistrictLength.NameLength} characters.");
        ...
        else if (model.Description.Length > DistrictLength.DescriptionLength)
            errors.Add($"Description {model.Description} must not exceed {DistrictLength.DescriptionLength} characters.");
        ...
        else if (model.Code.Length > DistrictLength.CodeLength)
            errors.Add($"Code {model.Code} must not exceed {DistrictLength.CodeLength} characters.");
